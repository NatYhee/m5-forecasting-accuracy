...
def make_dataset(raw_data_dir:str, export_dir:str, main_file:str, store_id:str = None):
...
def merge_dataset(sales_df:pd.DataFrame, calendar_df:pd.DataFrame, price_df:pd.DataFrame):
    sales_df = pd.melt(sales_df, id_vars=['id', 'item_id', 'dept_id', 'cat_id', 'store_id', 'state_id'], var_name="date_code", value_name ="sales")
    sales_df = pd.merge(left=sales_df, right=calendar_df, left_on='date_code', right_on='d', how='left')
    sales_df = pd.merge(left=sales_df, right=price_df, left_on=['store_id', 'item_id', 'wm_yr_wk'], right_on=['store_id', 'item_id', 'wm_yr_wk'], how='left')
    return sales_df
